...
void vizualizareCatalogUI(Biblioteca& bib) {
    ...
    for(size_t i=0; i<carti.size(); ++i) {
        ...
        rows.push_back({std::to_string(i+1), carti[i]->getTitlu(), carti[i]->autoriToString(), std::to_string(carti[i]->getStocDisponibil()), loc});
    }
    ...
    if(sel >= 0 && sel < (int)carti.size()) {
        ...
        // Since we don't have direct getRecenzii in Carte, we can use bib.afiseazaRecenziiCarte, but it prints to cout.
        // So we will just call it manually. Wait, afiseazaRecenziiCarte prints to cout and messes up UI.
        // We will just clear screen and call it.
...
void cautaCarteUI(Biblioteca& bib) {
    ...
    for(size_t i=0; i<rezultate.size(); ++i) {
        ...
        rows.push_back({std::to_string(i+1), rezultate[i]->getTitlu(), rezultate[i]->autoriToString(), std::to_string(rezultate[i]->getStocDisponibil()), loc});
    }
